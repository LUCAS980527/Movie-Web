...
export const PopularMovieList = () => {
  const [popularMovieListData, setPopularMovieListData] = useState([]);
  const [loading, setLoading] = useState(false);
  const getPopularMovieListData = async () => {
    setLoading(true);
    const popularMovieListDataEndpoint = `${BASE_URL}/movie/popular?language=en-US&page=1`;
    const response = await fetch(popularMovieListDataEndpoint, {
      headers: {
    ...
    setPopularMovieListData(data.results);
    setLoading(false);
  };
  useEffect(() => {
    getPopularMovieListData();
  }, []);
  ...
  return (
    <div className="flex items-center justify-center w-[1437px] h-[978px] flex-col mt-[53px] gap-[32px]">
      ...
      <div className=" gap-8 grid grid-cols-5  w-[1437px] h-[978px] px-[80px] box-border">
        {popularMovieListData.slice(0, 10).map((movie, index) => {
          return (
            <MovieCards
              key={index}
              title={movie.title}
